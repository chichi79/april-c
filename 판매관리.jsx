...
// ---------- storage ----------
const KEY = 'coupang-dropship:data-v2';
const DEFAULT_CATEGORIES = ['생활용품', '우산', '레저', '모자', '잡화', '시즌상품'];

const emptyData = () => ({
  products: [],   // {id, name, category, supplyPrice, sellPrice, fee, shipping, sourceUrl, status, createdAt}
  sales: [],      // {id, productId, date, qty, note}
  sourcing: [],   // {id, name, sourceUrl, supplyPrice, expectedSellPrice, note, status, createdAt}
    ...
    (async () => {
      try {
        ...
        if (res && res.value) {
          ...
          setDataState({ ...emptyData(), ...parsed, categories: parsed.categories?.length ? parsed.categories : DEFAULT_CATEGORIES });
        } else {
...
export default function App() {
  ...
  return <AppShell data={data} setData={setData} tab={tab} setTab={setTab} saveError={saveError} />;
}

function AppShell({ data, setData, tab, setTab, saveError }) {
  ...
  return (
    <div style={rootStyle}>
      ...
      <header style={{ borderBottom: '1px solid var(--ink-10)', padding: '18px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        ...
        {saveError && <div style={{ fontSize: 12, color: 'var(--red)', display: 'flex', alignItems: 'center', gap: 4 }}><AlertCircle size={14} />저장 실패 — 다시 시도해주세요</div>}
      </header>
